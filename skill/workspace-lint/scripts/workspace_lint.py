...
def check_dir_naming(root: Path, rules: dict, ignore_dirs: set = None) -> list[Violation]:
    ...
    ignore_set = ignore_dirs or set()

    seen_dirs = {}
    for dirpath, dirnames, filenames in os.walk(root):
        # Prune ignored directories so we don't flag build artifacts that are gitignored
        dirnames[:] = [d for d in dirnames if d not in ignore_set and not d.startswith(".")]
        for d in dirnames:
            full = Path(dirpath) / d
...
def check_file_placement(files: list[Path], root: Path, rules: dict) -> list[Violation]:
    ...
    # Whitelisted root files don't trigger preferred_dir warnings
    allowed_root = set(rules.get("root", {}).get("allowed_root_files", []))

    for f in files:
        rel = _relpath(f, root)
        # Skip files that are explicitly whitelisted at root
        ...
        for rule_pattern, rule_config in file_rules.items():
            ...
            if preferred:
                ...
                # Treat "." as "anywhere at root" — never warn
                if preferred_path == ".":
                    continue
...
def check_build_artifacts(root: Path, rules: dict, ignore_dirs: set = None) -> list[Violation]:
    ...
    for dirpath, dirnames, _ in os.walk(root):
        # Prune ignored directories — gitignored target/ should not be flagged
        dirnames[:] = [d for d in dirnames if d not in ignore_set and not d.startswith(".")]
        for d in dirnames:
