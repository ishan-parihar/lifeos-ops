    ...
    match cli.command {
        ...
        command => {
            ...
            match command {
                ...
                Commands::FillRate { database, limit, threshold } => {
                    let (cfg, notion, sc) = resolve_with_schema(None, &notion_token).await;
                    let params = lifeos_core::tools::fill_rate::FillRateParams {
                        database, limit, threshold,
                    };
                    match lifeos_core::tools::fill_rate::execute(&params, &cfg, &notion, &sc).await {
                        Ok(t) => println!("{t}"), Err(e) => { tracing::error!("{e}"); std::process::exit(1); }
                    }
                }
                Commands::QuickLink { source_db, source_title, target_db, target_title, property } => {
                    let (cfg, notion, sc) = resolve_with_schema(None, &notion_token).await;
                    let params = lifeos_core::tools::quick_link::QuickLinkParams {
                        source_db, source_title, target_db, target_title, property,
                    };
                    match lifeos_core::tools::quick_link::execute(&params, &cfg, &notion, &sc).await {
                        Ok(t) => println!("{t}"), Err(e) => { tracing::error!("{e}"); std::process::exit(1); }
                    }
                }
            }
